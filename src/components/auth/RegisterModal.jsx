  ...
  const submit = async () => {
    ...
    try {
      ...
      if (f.email) {
        ...
        setCodeSent(true);
        toast.ok('Регистрация успешна. Код подтверждения отправлен на email.');
      } else {
      ...
      <div className="modal">
        ...
        <div className="m-body">
          <div className="fg">
            ...
            <input className="fi" type="text" placeholder="Ваше имя" value={f.name} onChange={upd('name')} />
          </div>
          <div className="fg">
          ...
          <div className="f-check">
            ...
            <label htmlFor="ag">Я принимаю условия использования и политику конфиденциальности</label>
          </div>
          <button className="submit" onClick={submit} disabled={loading || !f.name || !f.pass}>
            {loading ? 'Создаем аккаунт...' : 'Зарегистрироваться'}
          </button>
          {codeSent && <p style={{ marginTop: 10, color: 'var(--gold)', fontSize: 12 }}>Откройте личный кабинет и подтвердите email кодом.</p>}
        </div>
        <div className="m-ftr">
