    ...
    const handleSocialCallback = async () => {
      try {
        ...
        if (isTelegramMagic) {
          ...
          if (!token) throw new Error('Не передан токен входа Telegram');
          const userData = await api.auth.consumeTelegramMagic(token);
          login(userData);
          toast.ok(`Добро пожаловать, ${userData.name || 'гость'}!`);
        ...
        if (isGoogle) {
          ...
          if (!code) throw new Error('Не получен code от Google');
          userData = await api.auth.loginWithGoogle(code);
        } else if (isVk) {
          const code = params.get('code');
          if (!code) throw new Error('Не получен code от VK');
          const redirectUri = `${window.location.origin}/auth/vk/callback`;
          if (isLinkFlow) {
            const targetUserId = Number(state.split(':')[1] || 0);
            if (!targetUserId) throw new Error('Некорректный state привязки VK');
            await api.auth.linkVk(targetUserId, code, redirectUri);
            toast.ok('VK успешно привязан');
            window.history.replaceState({}, '', '/');
            return;
          }
          ...
          if (!tg.id || !tg.hash || !tg.auth_date) throw new Error('Некорректные данные Telegram');
          const tgPayload = {
            id: String(tg.id),
          ...
          if (isLinkFlow) {
            const targetUserId = Number(state.split(':')[1] || 0);
            if (!targetUserId) throw new Error('Некорректный state привязки Telegram');
            await api.auth.linkTelegram(targetUserId, tgPayload);
            toast.ok('Telegram успешно привязан');
            window.history.replaceState({}, '', '/');
            return;
          }
        ...
        if (!userData) throw new Error('Не удалось завершить вход');
        login(userData);
        toast.ok(`Добро пожаловать, ${userData.name || 'гость'}!`);
        window.history.replaceState({}, '', '/');
      } catch (e) {
        toast.err(e.message || 'Не удалось выполнить вход через соцсеть');
        window.history.replaceState({}, '', '/');
      }
    };
